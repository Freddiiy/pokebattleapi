...
def predict_battle(pokemons):
    df = pd.read_csv("./poke_data/data/v2/battle_data.csv")
    ...
    # Original data
    nn = MLPClassifier(activation='logistic', solver='adam', hidden_layer_sizes=(8, 8, 8, 8), max_iter=500, random_state=42)
    nn.fit(X_train, y_train)
    pred = nn.predict(X_test)
    score = nn.score(X_test, y_test)

    # Normalized data
    nn_norm = MLPClassifier(activation='logistic', solver='adam', hidden_layer_sizes=(8, 8, 8, 8), max_iter=500, random_state=42)
    scaler = MinMaxScaler()
    ...
    pred_norm = nn_norm.predict(pokemons)
    score_norm = nn_norm.score(X_norm_test, y_test)

    print(pred_norm)
    return [pred_norm, score]
...
# original 0.8452030606238964
# normalize 0.9217186580341378