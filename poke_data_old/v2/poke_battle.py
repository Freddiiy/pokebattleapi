...
from poke_data_old.ApiToCsv import read_pokemon_data, read_battles_data
import pandas as pd
import numpy as np
...
def battle_pokemon(first: int, second: int):
    df = read_pokemon_data("./poke_data_old/data/pokemon.csv")
...
def write_all_battles():
    ...
    with open("./poke_data_old/data/v2/match.csv", "w") as file:
...
def calc_battles_diff():
    pokemons = read_pokemon_data("/poke_data_old/data/pokemon.csv")
    battle_results = read_battles_data("/poke_data_old/data/v2/match.csv")
...
def calc_battles():
    ...
    df.to_csv('./poke_data_old/data/v2/battle_data.csv', index=False, header=header)
